    ...
    public class RenderMonsterEditorWindow : Editor
    {
...
#else
                icon_screenshot = UnityEditor.EditorGUIUtility.IconContent("Texture Icon").image;
#endif
        }
        ...
        void DrawSettings()
        {
            ...
            using (new AmazingAssets.EditorGUIUtility.EditorGUIIndentLevel(1))
            {
                ...
                {
                    using (new AmazingAssets.EditorGUIUtility.GUIBackgroundColor(IsOutputFolderValid() ? Color.white : Color.red))
                    {
                    ...
                    {
                        if (IsOutputFolderValid() && Event.current.button == 1)
                        {
                            _target.OpenSaveFolder();
                        }
                        else 
                        { 
                            string path = UnityEditor.EditorUtility.SaveFolderPanel("Save textures to directory", IsOutputFolderValid() ? outputPath.stringValue : "", "");

                            if (string.IsNullOrEmpty(path) == false)
                            {
                                if (HasDirectorySavePermition(path))
                                {
                                    outputPath.stringValue = path;
                                }
                                else
                                {
                                    UnityEditor.EditorUtility.DisplayDialog("Destination Folder Access Denied", "You have no permission to save files in this folder.", "Ok");
                                }
        ...
        bool HasDirectorySavePermition(string _path)
        {
            byte[] bytes = new byte[] { 0, 1, 2, 3 };

            string fullPath = Path.Combine(_path, "__TestFile__.txt");
        ...
        {
            if (string.IsNullOrEmpty(outputPath.stringValue) || Directory.Exists(outputPath.stringValue) == false)
                return false;
            else
                return true;
        }
        ...
        public string CorrectName(string _str)
        {
            ...
            {
                _str = _str.Replace(c, string.Empty);
            }
